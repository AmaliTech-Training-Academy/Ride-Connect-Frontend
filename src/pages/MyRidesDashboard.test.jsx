...
describe('MyRidesDashboard - Ride Status Management', () => {
  ...
  describe('rides I have joined', () => {
    ...
    it('shows a declined request with a reason and offers to rejoin', async () => {
      ...
      payload.data.joined = [
        joinedRide({
          ...
          rejectionReason: 'Ride is full for this route already.',
          rerequestCount: 0,
        }),
      ]
      fetchMyRides.mockResolvedValue(payload)
      ...
      expect(
        screen.getByText(/Ride is full for this route already\./),
      ).toBeInTheDocument()
      expect(
    ...
    it('does not offer another request once the decline is final', async () => {
      ...
      const payload = buildMyRidesResponse()
      payload.data.joined = [
        joinedRide({ requestStatus: 'DECLINED', rerequestCount: 1 }),
      ]
      fetchMyRides.mockResolvedValue(payload)
      await renderDashboard()
